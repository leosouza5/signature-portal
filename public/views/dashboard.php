...
$inProgress = count(array_filter($documents, fn ($item) => in_array($item['status'], ['DRAFT', 'SENT'], true)));
$signed = count(array_filter($documents, fn ($item) => $item['status'] === 'COMPLETED'));
$recent = array_slice($documents, 0, 4);
...
<div class="dashboard-title-row">
    <h1>Dashboard</h1>
    <a class="new-doc-button" href="/documentos/criar"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg>Novo documento</a>
</div>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($recent as $document): ?>
                        <tr>
                            <td><span class="doc-name"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"/><path d="M14 2v6h6"/></svg><?= e($document['title']) ?></span></td>
                            <td><span class="dashboard-badge <?= e(dashboard_status_class($document['status'])) ?>"><?= e(dashboard_status_label($document['status'])) ?></span></td>
                            <td><?= e($document['created_at']) ?></td>
                            <td><a class="view-action" href="/documentos/<?= e((string) $document['id']) ?>" aria-label="Ver documento"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></a></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
